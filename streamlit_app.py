...
with st.form("add_client", clear_on_submit=True):
    ...
    with st.expander("**Weekly Schedule** (click to set working hours)", expanded=False):
        ...
        for day in DAYS:
            ...
            with col2:
                start_h = st.number_input("Start Hour", min_value=0, max_value=23, value=8, key=f"start_{day}", label_visibility="collapsed")
            with col3:
                end_h = st.number_input("End Hour", min_value=0, max_value=23, value=22, key=f"end_{day}", label_visibility="collapsed")
